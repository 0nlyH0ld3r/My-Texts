...
Uma explicação breve para funções é: 
...
ponteiro para char (que devolve o endereço de memória para uma string), e até mesmo **void**, caso não retorne nada (Será tratado em um próximo tópico).
...
* Mas, para quem quiser saber: Um sistema operacional não funciona assim (obviamente),
...
com a função system() para executar comandos do sistema (presente na stdlib.h) */
...
tudo o que foi necessário para os argumentos passados ou determinado valor passado seja a condição para parar a função)
...
int numero_certo(int x) {
    if (x != 4) return 0;       /* A função para se o número não for 4 e retorna 0 para indicar falso, ou seja, que não foi o certo. */
...
basta mudar o tipo para void e tirar o valor returnado pelo **return**, deixando ele puro para "cancelar" a chamada da função).
...
Ou seja: Funções que somente alteram alguma coisa. Geralmente são funções que recebem variáveis para alterar seu valor, funções que somente realizam ações
simples e que não há necessidade de receber algum valor para determinar o resultado a execução (Como strncpy que retorna 0 caso as strings sejam iguais e etc).
...
posto dentro da função (o que for posto como argumento) durante a sua chamada (tem exemplo abaixo para explicar).

Argumentos são variáveis (ou constantes) colocadas entre os parênteses da função durante a sua chamada para dar valor aos parâmetros dentro da função.
ex:
...
Funções também podem ser criadas sem parâmetros. Para criar funções sem parâmetros, basta não declarar parâmetro algum ou adicionar **void** (vazio) como parâmetro.
ex:
...
apesar de não fazer diferença alguma no código aparentemente. */
...
Passar literalmente a função (pondo seu nome sem os parênteses após ele).
...
Sei que passar uma função como argumento de uma função argumento de outra função parece meio feio, mas não é tão incomum, apesar de passar uma função como argumento
de outra função (2 camadas) ser bem comum (Isso não significa que não deva ser utilizado assim, somente que EU não vi muitas. Mas meu repertório não é tão grande assim
para ditar qual o padrão do mercado).
...
Além disso, as funções void não podem ser passadas pelo primeiro método, mas podem por esse. Só há o fato de que não irão dar valor a nada. 
Melhor pesquisar para entender o que acontece aqui.
...
// Um exemplo printando o endereço de x() para demonstrar que funções também o possuem.
...
(Lembrete de que, para acessar o endereço de uma variável é necessário usar &variavel, mas para ponteiros, vetores, matrizes e funções, não. Basta apenas por seu nome,
com exceção de elementos em específico de um vetor, ou seja, um vetor indexado vetor[0], vetor[4] e etc).
...
O parâmetro de uma função é algo totalmente separado do argumento. Quando você passa um argumento para uma função (como no exemplo abaixo)
...
Assim como já deve ter sido lido no outro arquivo que trata sobre ponteiros: Ponteiros são variáveis que recebem endereços de memória.
Com esse endereço é possível alterar o valor de uma variável.
...
O conceito em si não diverge de uma chamada por valor, entretanto, como o VALOR de um ponteiro é um ENDEREÇO, um ENDEREÇO será passado e por meio de um endereço é
...
fatal. O trabalho para achar isso seria gigantesco. Mas, e se fosse ainda maior? Centenas de arquivos e 20 funções concorrendo pela mesma variável?
...
forma de alcançar o mesmo resultado. É realmente necessário? O hardware por onde seu sistema passará é limitado o bastante para te fazer poupar o máximo de memória
possível (apesar de que é bom sim poupar memória, tornar o código complexo demais de se manter não é uma boa opção se há recursos o suficiente para utilizar um pouco a
